# AI Agent Core Routing Hub — Aegis Memory

> 작업 시작 전 반드시 이 파일을 먼저 읽고 규칙을 로드한다.

이 파일은 `slicerag`에서 에이전트가 로드해야 할 로컬 규칙의 인덱스다.
...
2. `agents/rules/00-L2-common.md`
   - Aegis 공통 보안/문서/작업 규칙을 확인한다.
3. `agents/rules/10-L3-slicerag.md`
   - Project RAG, ingest, embedding, search API 작업 시 확인한다.

## 핵심 경계

`slicerag`는 외부 클라이언트 진입점이 아니다.

```text
외부 공개: aegis-gateway
내부 호출: slicerag
모델 호출: aegis-gateway 정책 경유
```

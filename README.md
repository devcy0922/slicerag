...
- `SLICERAG_STORE`: 사용할 스토어 엔진 유형 (`postgres` 또는 `memory`).
- `SLICERAG_DATABASE_URL`: PostgreSQL 데이터베이스 접속 문자열 (`postgres` 모드 전용).
- `SLICERAG_EMBEDDING_PROVIDER`: 임베딩 생성 방식 (`hash` 또는 `openai`).

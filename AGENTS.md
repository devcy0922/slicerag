# Aegis Memory — AI Agent Guide

작업 시작 전에는 반드시 `agents/rules/rules.md`를 먼저 읽고 프로젝트 규칙을 로드한다.

이 저장소는 Aegis 플랫폼의 내부 Project RAG 컴포넌트다. 외부 클라이언트의 진입점이 아니며, `aegis-gateway`가 인증과 프로젝트 스코프를 확인한 뒤 내부 서비스로 호출한다.
...
- 외부 요청 인증은 `aegis-gateway` 책임이다.
- `slicerag`는 내부 네트워크에서만 접근 가능한 서비스로 배포한다.
